...
:intro-paragraph: {product-name} is derived from a vendor neutral, https://cncf.io[CNCF] Sandbox project, called link:https://kubewarden.io[{community-product-name}], originally created by https://www.rancher.com/[SUSE Rancher]. The name {community-product-name} refers to the open-source community project. The name {community-product-name} may also be used throughout this documentation in place of {product-name}.
endif::[]
