...
Les invocations suivantes sont rapides car {admc-name} utilise la fonctionnalité de cache de Wasmtime.
La première exécution peut prendre environ 20 secondes avec `kwctl`, les exécutions suivantes, 1 à 2 secondes.
Ainsi, le serveur de stratégie {admc-short-name} a un temps de démarrage plus lent mais ensuite les temps d'évaluation des stratégies ne sont généralement pas affectés par l'utilisation d'objets Kubernetes.
====
...
. Créez un objet `payload`. Faites cela en utilisant une fonction d'assistance du SDK {admc-short-name} : `kubewarden_testing.BuildValidationRequest`.
...
Cela ne se produit pas lorsque la stratégie s'évalue dans {admc-short-name}, dans ce contexte, les déclarations `logger` émettent des événements OpenTelemetry à la place.
====
