...
:description: Configurez le {admc-name} pour envoyer en toute sécurité des données de télémétrie à un collecteur OpenTelemetry personnalisé déployé au sein du cluster.
...
Ce guide explique comment configurer {admc-short-name} pour envoyer des données de télémétrie à un collecteur OpenTelemetry déjà déployé sur le cluster.
...
Vous avez besoin d'une communication chiffrée entre les composants {admc-name} et le collecteur. Vous pouvez utiliser https://cert-manager.io/[cert-manager] pour gérer tous les certificats nécessaires à une communication sécurisée.

Les traces du Collecteur OpenTelemetry sont envoyées à une instance https://www.jaegertracing.io/[Jaeger].

La pile {admc-short-name} envoie des métriques au Collecteur OpenTelemetry.
...
Vous configurez la communication entre les composants {admc-short-name} et le Collecteur OpenTelemetry en utilisant mTLS.
...
Le moniteur de service Prometheus obtient les métriques {admc-short-name} en récupérant le collecteur OpenTelemetry fonctionnant dans l'espace de noms `kubewarden`.
...
Vous sécurisez la communication entre la pile {admc-short-name} et le Collecteur OpenTelemetry en utilisant mTLS. Cependant, la communication entre le Collecteur OpenTelemetry et Jaeger n'est pas sécurisée, afin de réduire la complexité de l'exemple.

== Installez la pile {admc-short-name}

Lorsque le Collecteur OpenTelemetry est en cours d'exécution, vous pouvez déployer {admc-short-name} de la manière habituelle.

Vous devez configurer les composants {admc-short-name} afin qu'ils envoient des événements et des métriques au Collecteur OpenTelemetry.
...
Le Secret référencé par la clé `otelCollectorClientCertificateSecret` doit avoir les entrées suivantes : les clés `tls.crt` et `tls.key`. Ce sont le certificat client et sa clé utilisés par la pile {admc-short-name} pour s'authentifier auprès du Collecteur OpenTelemetry.
...
Vous pouvez voir les événements de trace générés par {admc-short-name} en utilisant l'interface web de Jaeger. Ils sont regroupés sous le service `kubewarden-policy-server` :
