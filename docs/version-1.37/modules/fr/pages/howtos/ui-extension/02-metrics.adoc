...
:description: Configurez la surveillance des environnements Rancher en intégrant le {admc-name} et Prometheus.
:doc-persona: ["kubewarden-operator", "kubewarden-integrator"]
:doc-topic: ["operator-manual", "metrics-and-monitoring"]
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "extension d'interface utilisateur", "métriques", "surveillance"]
:current-version: {page-origin-branch}

Rancher dispose d'un outil de cluster pour la surveillance qui utilise Grafana et Prometheus. Vous pouvez utiliser cet outil, en l'intégrant avec {admc-name}, pour voir les métriques globales d'un Serveur de Politiques ou les métriques d'une Politique donnée.
...
* Vous devez spécifier l'espace de noms d'installation de {admc-short-name}
...
Vous devriez pouvoir voir les métriques pour un Serveur de Politiques, ou toute Politique donnée sur la page de détail de chaque ressource respective. Vous pouvez également voir {admc-short-name} les tableaux de bord dans l'interface Grafana, ou les événements depuis l'interface Prometheus.
