...
= {admc-name} dans un environnement à grande échelle
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-06-18
:page-revdate: {revdate}
:description: Configurer les {admc-name} avec {admc-name} pour garantir une haute disponibilité et des performances dans des environnements à grande échelle exigeants.
...
Cette section détaille un déploiement réel de {admc-name} dans un environnement exigeant à grande échelle. Elle illustre comment configurer {admc-short-name} pour une haute disponibilité et des performances, et ce à quoi s'attendre sous une charge importante.

[IMPORTANT]
====
Si vous souhaitez voir plus de conseils sur la façon d'exécuter {admc-short-name} en production, consultez la documentation sur les xref:/howtos/production-deployments.adoc[Déploiements en production].
====
...
Pour répondre aux exigences de cet environnement, {admc-short-name} est configuré avec un accent sur l'isolation des charges de travail et la haute disponibilité.
