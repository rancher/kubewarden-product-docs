...
:title: kwctl CLI
:description: Erforschen Sie das \`kwctl` Kommandozeilenwerkzeug zur Verwaltung von {admc-name} Richtlinien und zur Durchführung von Aufgaben wie das Laden von Richtlinien.
...
Werkzeug zur Verwaltung von {admc-name} Richtlinien

*Verwendung:* `kwctl [OPTIONS] <COMMAND>`

==== Unterbefehle

* `annotate` — Fügt {admc-short-name} Metadaten zu einem WebAssembly-Modul hinzu
* `bench` — Führt Benchmarktests an einer {admc-short-name} Richtlinie durch
...
* `inspect` — Untersucht die {admc-short-name} Richtlinie
* `load` — Lädt Richtlinien aus einer tar.gz-Datei
* `policies` — Listet alle heruntergeladenen Richtlinien auf
* `pull` — Holt eine {admc-short-name} Richtlinie von einer angegebenen URI
* `push` — Schiebt eine {admc-short-name} Richtlinie in ein OCI-Registry
* `rm` — Entfernt eine {admc-short-name} Richtlinie aus dem Speicher
* `run` — Führt eine {admc-short-name} Richtlinie von einer gegebenen URI aus
* `save` — Speichert Richtlinien in einer tar.gz-Datei
* `scaffold` — Erstellt eine Kubernetes-Ressource oder Konfigurationsdatei
* `verify` — Überprüft eine {admc-short-name} Richtlinie von einer gegebenen URI mit Sigstore
...
Fügen Sie {admc-short-name} Metadaten zu einem WebAssembly-Modul hinzu
...
Führt Benchmarktests an einer {admc-short-name} Richtlinie durch.
...
- Lokale YAML-Datei: z.B. `file://home/tux/cluster-admission-policy.yaml` (enthält Deklarationen von {admc-short-name} benutzerdefinierten Ressourcen wie `ClusterAdmissionPolicy`, `AdmissionPolicy` usw.)

Standardverhalten:
Wenn das Schema weggelassen wird, wird `file://` angenommen, das im aktuellen Verzeichnis verwurzelt ist.

Hinweise zu {admc-short-name} benutzerdefinierten Ressourcen:
...
- Das `--raw` Flag kann nicht verwendet werden, da die benutzerdefinierten Ressourcen von {admc-short-name} `raw` Richtlinien nicht unterstützen.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — Richtlinien-URI, SHA-Präfix oder YAML-Datei, die {admc-short-name} Richtlinienressourcen enthält. Unterstützte Schemas: registry://, https://, file://. Wenn das Schema weggelassen wird, wird file:// angenommen, basierend auf dem aktuellen Verzeichnis.
...
Untersuche die {admc-short-name} Richtlinie
...
Holt eine {admc-short-name} Richtlinie von einer gegebenen URI
...
* `--github-owner <VALUE>` — Erwarteter GitHub-Besitzer in den in CD-Pipelines generierten Zertifikaten
* `--github-repo <VALUE>` — Erwartetes GitHub-Repository in den in CD-Pipelines generierten Zertifikaten
* `-o`, `--output-path <PATH>` — Ausgabedatei. Wenn nicht angegeben, wird sie im {admc-short-name} Store heruntergeladen
* `--sigstore-trust-config <PATH>` — JSON-formatierte Datei, die der ClientTrustConfig-Nachricht in den Sigstore-Protobuf-Spezifikationen entspricht. Diese Datei konfiguriert den gesamten Zustand der Sigstore-Instanz, einschließlich der URIs, die zum Zugriff auf die CA und die Dienste für die Transparenz von Artefakten verwendet werden, sowie des kryptografischen Vertrauensankers selbst.
* `--sources-path <PATH>` — YAML-Datei, die Quellinformationen enthält (https, unsichere Registrierungs-Hosts, benutzerdefinierte CAs...)
* `-a`, `--verification-annotation <KEY=VALUE>` — Annotation im key=value-Format. Kann mehrfach wiederholt werden
* `--verification-config-path <PATH>` — YAML-Datei, die Informationen zur Verifizierungskonfiguration enthält (Signaturen, öffentliche Schlüssel...)
...
Überträgt eine {admc-short-name} Richtlinie an das OCI-Registry
...
Entfernt eine {admc-short-name} Richtlinie aus dem Store
...
Führen Sie eine oder mehrere {admc-short-name} Richtlinien lokal aus.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — Policy-URI, SHA-Präfix oder YAML-Datei, die {admc-short-name} Richtlinienressourcen enthält. Unterstützte Schemas: registry://, https://, file://. Wenn das Schema weggelassen wird, wird file:// angenommen, basierend auf dem aktuellen Verzeichnis.
...
* `vap` — Konvertieren Sie ein Kubernetes `ValidatingAdmissionPolicy` in ein {admc-short-name} `ClusterAdmissionPolicy`
...
* `-o`, `--operation <TYPE>` — {admc-short-name} benutzerdefinierter Ressourcentyp +
...
* `-t`, `--type <VALUE>` — {admc-short-name} benutzerdefinierter Ressourcentyp +
...
Konvertieren Sie ein Kubernetes `ValidatingAdmissionPolicy` in ein {admc-short-name} `ClusterAdmissionPolicy`
...
Überprüfen Sie eine {admc-short-name} Richtlinie von einer gegebenen URI unter Verwendung von Sigstore
