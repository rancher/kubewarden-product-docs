...
:keywords: ["{admc-name}", "kubewarden", "kontextabhängige Richtlinie", "clusteradmissionpolicy", "admissionpolicy", "admissionrequest", "clusteradministrator"]
...
[NOTE]
====
Kontextabhängige Richtlinien sind nur in {admc-name} Versionen ≥ v1.6.0 verfügbar.
====
...
Standardmäßig blockiert {admc-short-name} alle Clusterressourcen. Ein {admc-name} Administrator definiert, auf welche Kubernetes-Ressourcen jede kontextabhängige Richtlinie zugreifen kann. Dafür wird `contextAwareResources` verwendet.
...
Richtlinienautoren stellen Listen von Kubernetes-Ressourcen für ihre kontextabhängige Richtlinie bereit. Richtlinienautoren tun dies, indem sie die Richtlinie annotieren. {admc-name} Administratoren sehen die Metadaten der Richtlinie mit dem `kwctl inspect` Befehl ein. Sie können eine Liste der Ressourcen abrufen, auf die die Richtlinie zugreifen muss. Ein Administrator verwendet diese Liste, um die `ClusterAdmissionPolicy` Definition auszufüllen.

[WARNING]
====

Um Missbrauch des Systems zu verhindern, müssen {admc-name} Administratoren die Ressourcen überprüfen, auf die die Richtlinie zugreift.
