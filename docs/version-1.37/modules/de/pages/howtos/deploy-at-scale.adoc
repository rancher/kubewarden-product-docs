...
= {admc-name} in einer großangelegten Umgebung
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-06-18
:page-revdate: {revdate}
:description: Konfigurieren Sie {admc-name}s mit {admc-name}, um hohe Verfügbarkeit und Leistung in anspruchsvollen großangelegten Umgebungen sicherzustellen.
:keywords: [kubewarden, "{admc-name}", kubernetes, policyservers, produktion]
...
Dieser Abschnitt beschreibt eine reale Implementierung von {admc-name} in einer anspruchsvollen, großangelegten Umgebung. Es wird veranschaulicht, wie man {admc-short-name} für hohe Verfügbarkeit und Leistung konfiguriert und was man unter hoher Last erwarten kann.

[IMPORTANT]
====
Wenn Sie weitere Tipps sehen möchten, wie Sie {admc-short-name} in der Produktion ausführen können, werfen Sie einen Blick in die xref:/howtos/production-deployments.adoc[Produktionsimplementierungen] Dokumentation.
====
...
== Die {admc-short-name}-Konfiguration

Um den Anforderungen dieser Umgebung gerecht zu werden, wird {admc-short-name} mit einem Fokus auf Arbeitslastisolierung und hohe Verfügbarkeit konfiguriert.
