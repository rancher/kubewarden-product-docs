...
= Verwendung von Pod-Sicherheitszulassung mit {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-06-18
:page-revdate: {revdate}
:description: Konfigurieren Sie die Pod-Sicherheitszulassung in einem Kubernetes-Cluster mit {admc-name}, um Pods abzusichern.
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "pod-sicherheitszulassung", "pod-sicherheitsrichtlinie", "kubernetes"]
...
{admc-short-name} kann verwendet werden, um ein PSA-Profil zu *integrieren* und so diese Einschränkungen zu vermeiden.

[NOTE]
====

Sie könnten {admc-short-name} verwenden, um die alte PSP-Konfiguration zu ersetzen, wie in xref:howtos/psp-migration.adoc[PSP-Migration] gezeigt.
Das Ziel dieses Artikels ist es jedoch zu zeigen, wie {admc-short-name} die neue PSA ergänzen kann.
====
...
Hier kann {admc-short-name} helfen.
...
Für dieses Beispiel benötigen Sie den {admc-short-name} Stack im Kubernetes-Cluster.
...
Jetzt wird die Bereitstellung durch die Richtlinie von {admc-short-name} verändert, die es erlaubt, dass der Pod gestartet wird.
...
Die {admc-short-name}-Integration kann in diesem Szenario mehr tun.
Es kann den Wert des bereitgestellten `runAsUser` überprüfen.

Diese Ressource wird von der vorherigen {admc-short-name}-Richtlinie abgelehnt:
...
Die Verwendung von {admc-short-name} zusammen mit PSA hilft, diese Lücke zu schließen.
