...
Folgende Aufrufe sind schnell, da {admc-name} die Cache-Funktion von Wasmtime nutzt.
Die erste Ausführung kann etwa 20 Sekunden mit `kwctl` dauern, spätere Ausführungen 1 bis 2 Sekunden.
Die {admc-short-name} Policy Server hat eine langsamere Startzeit, aber die Zeiten der Richtlinienbewertung werden normalerweise nicht durch die Verwendung von Kubernetes-Objekten beeinflusst.
====
...
. Erstellen Sie ein `payload`-Objekt. Tun Sie dies mit einer Hilfsfunktion des {admc-short-name} SDK: `kubewarden_testing.BuildValidationRequest`.
...
Das passiert nicht, wenn die Richtlinie in {admc-short-name} ausgewertet wird; in diesem Kontext geben die `logger` Anweisungen stattdessen OpenTelemetry-Ereignisse aus.
====
