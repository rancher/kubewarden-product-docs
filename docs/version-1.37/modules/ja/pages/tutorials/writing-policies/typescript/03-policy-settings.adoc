...
:description: src/types.tsおよびsrc/index.tsファイル内の{admc-name}ポリシーを通じて、効果的なポリシー設定構造を定義し、実装します。
:keywords: "{admc-name}", kubewarden, kubernetes, ポリシー設定の定義, TypeScript
:doc-type: チュートリアル
:doc-topic: {admc-name}, kubewarden, ポリシーの作成, typescript, ポリシー設定の定義
...
STDOUTへの書き込みはポリシーが正しく動作しなくなります。代わりに、ログ記録にはSTDERRを使用するか、{admc-name} SDKが提供するログ記録機能を使用してください。ポリシーのSTDOUTへの出力は、検証応答のみを含む必要があります。
====
...
{admc-short-name}ポリシーは、設定を処理する2つの関数を使用します：
