...
これを実現するために、ポリシーに含まれているhttps://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name}のホスト機能のためのCEL拡張機能ライブラリ]を使用できます。
...
今、クラスター内の既存のIngressを取得するCELコードを書く必要があります。そのために、[{admc-short-name} CEL拡張機能ライブラリ](https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities)を使用します。
