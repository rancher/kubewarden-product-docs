...
{admc-short-name}には、クラスター内での{admc-short-name}の動作についての概要を示す基本的なメトリクスを持つGrafanaダッシュボードがあります。このダッシュボードは、{admc-short-name}ポリシーサーバーリポジトリのGitHubリリースで、https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[JSONファイル]として、またはhttps://grafana.com/grafana/dashboards/15314[Grafanaウェブサイト]で利用可能です。
