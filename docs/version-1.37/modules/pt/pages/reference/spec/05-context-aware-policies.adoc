...
O Servidor de Políticas `default` implantado pelos gráficos Helm {admc-name} tem acesso aos seguintes recursos do Kubernetes:
...
| Rego
| ✅
| Desde o lançamento {admc-short-name} 1.9

| WASI
| ✅
| Desde a versão {admc-short-name} 1.10.0, apenas para o SDK Go
|===

== Restrições

A prioridade de {admc-short-name} é reduzir o número de consultas ao servidor API do Kubernetes. {admc-short-name} considera duas restrições:
...
As políticas de {admc-short-name} podem buscar múltiplos recursos ao mesmo tempo. Por exemplo, elas podem fazer uma consulta como "obter todos os Pods definidos dentro do namespace `default` que têm o rótulo `color` definido como ``green``".
...
O Policy Server cria duas watches e duplica todos os Pods da segunda consulta. Essa limitação será removida em futuras versões do {admc-short-name}.

=== Buscando um recurso específico

As políticas de {admc-short-name} podem obter um recurso específico definido dentro do cluster.
...
O comportamento da consulta direta ou em cache é configurado em nível de consulta pelo autor da política usando os SDKs do {admc-short-name}.
