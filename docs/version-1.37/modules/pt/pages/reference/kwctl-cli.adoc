...
:title: kwctl CLI
:description: Explore a ferramenta de linha de comando \`kwctl` para gerenciar {admc-name} políticas e realizar tarefas como carregar políticas.
...
Ferramenta para gerenciar {admc-name} políticas

*Uso:* `kwctl [OPTIONS] <COMMAND>`

==== Subcomandos

* `annotate` — Adiciona metadados {admc-short-name} a um módulo WebAssembly
* `bench` — Avalia uma política {admc-short-name}
...
* `inspect` — Inspeciona a política {admc-short-name}
* `load` — Carrega políticas de um arquivo tar.gz
* `policies` — Lista todas as políticas baixadas
* `pull` — Puxa uma política {admc-short-name} de uma URI fornecida
* `push` — Envia uma política {admc-short-name} para um registro OCI
* `rm` — Remover uma política {admc-short-name} do armazenamento
* `run` — Executa uma política {admc-short-name} de uma URI fornecida
* `save` — Salva políticas em um arquivo tar.gz
* `scaffold` — Cria um recurso ou arquivo de configuração Kubernetes
* `verify` — Verifica uma política {admc-short-name} de uma URI fornecida usando Sigstore
...
Adiciona {admc-short-name} metadados a um módulo WebAssembly
...
Avalia uma política {admc-short-name}.
...
- Arquivo YAML local: por exemplo, `file://home/tux/cluster-admission-policy.yaml` (contém declarações de Recursos Customizados {admc-short-name} como `ClusterAdmissionPolicy`, `AdmissionPolicy`, etc.)

Comportamento Padrão:
Se o esquema for omitido, assume-se `file://`, enraizado no diretório atual.

Notas sobre Recursos Customizados {admc-short-name}:
...
- A flag `--raw` não pode ser usada, pois os Recursos Customizados de {admc-short-name} não suportam políticas `raw`.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — URI da política, prefixo SHA ou arquivo YAML contendo recursos de política {admc-short-name}. Esquemas suportados: registry://, https://, file://. Se o esquema for omitido, file:// é assumido, com base no diretório atual.
...
Inspeciona {admc-short-name} a política
...
Puxa {admc-short-name} uma política de uma URI dada
...
* `--github-owner <VALUE>` — Proprietário do GitHub esperado nos certificados gerados em pipelines de CD
* `--github-repo <VALUE>` — Repositório do GitHub esperado nos certificados gerados em pipelines de CD
* `-o`, `--output-path <PATH>` — Arquivo de saída. Se não fornecido, será baixado para a loja {admc-short-name}
* `--sigstore-trust-config <PATH>` — Arquivo formatado em JSON que conforma à mensagem ClientTrustConfig nas especificações protobuf do Sigstore. Este arquivo configura todo o estado da instância do Sigstore, incluindo os URIs usados para acessar os serviços de CA e transparência de artefatos, bem como a raiz de confiança criptográfica em si
* `--sources-path <PATH>` — Arquivo YAML contendo informações de origem (https, hosts inseguros do registro, CAs personalizadas...)
* `-a`, `--verification-annotation <KEY=VALUE>` — Anotação no formato chave=valor. Pode ser repetido várias vezes
* `--verification-config-path <PATH>` — Arquivo YAML contendo informações de configuração de verificação (assinaturas, chaves públicas...)
...
Envia uma política {admc-short-name} para um registro OCI
...
Remove uma política {admc-short-name} da loja
...
Execute uma ou mais políticas {admc-short-name} localmente.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — URI da política, prefixo SHA ou arquivo YAML contendo recursos de política {admc-short-name}. Esquemas suportados: registry://, https://, file://. Se o esquema for omitido, file:// é assumido, enraizado no diretório atual.
...
* `vap` — Converter um `ValidatingAdmissionPolicy` do Kubernetes em um {admc-short-name} `ClusterAdmissionPolicy`
...
* `-o`, `--operation <TYPE>` — Tipo de Recurso Customizado {admc-short-name} +
...
* `-t`, `--type <VALUE>` — Tipo de Recurso Customizado {admc-short-name} +
...
Converter um `ValidatingAdmissionPolicy` do Kubernetes em um {admc-short-name} `ClusterAdmissionPolicy`
...
Verifique uma política {admc-short-name} de uma URI fornecida usando o Sigstore
