...
:description: Defina e implemente estruturas eficazes de configurações de políticas por meio das {admc-name} políticas nos arquivos src/types.ts e src/index.ts.
:keywords: "{admc-name}", Kubewarden, kubernetes, definindo configurações de políticas, TypeScript
:doc-type: tutorial
:doc-topic: {admc-name}, Kubewarden, escrevendo-políticas, typescript, definindo-configurações-de-políticas
...
Escrever no STDOUT quebra as políticas. Em vez disso, use STDERR para logging ou a ferramenta de logging fornecida pelo {admc-name} SDK. A saída da política para o STDOUT deve conter apenas a resposta de validação.
====
...
As {admc-short-name} políticas usam duas funções que lidam com as configurações:
