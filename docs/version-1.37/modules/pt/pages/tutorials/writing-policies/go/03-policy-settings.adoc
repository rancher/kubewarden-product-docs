...
Uma política {admc-short-name} expõe duas funções diferentes que recebem as configurações de política como entrada:
...
* `validate_settings`: Chame esta função quando a política for carregada pela primeira vez por {admc-short-name}.
...
Todas as políticas de {admc-short-name} devem implementar a validação de configurações.
...
Você pode ver que a função aproveita as funções auxiliares fornecidas pelo SDK de https://github.com/kubewarden/policy-sdk-go[{admc-short-name}.]
