...
As invocações seguintes são rápidas porque {admc-name} usa o recurso de cache do Wasmtime.
A primeira execução pode levar cerca de 20 segundos com `kwctl`, execuções posteriores, de 1 a 2 segundos.
Portanto, o {admc-short-name} Policy Server tem um tempo de inicialização mais lento, mas os tempos de avaliação da política geralmente não são afetados pelo uso de objetos Kubernetes.
====
...
. Crie um objeto `payload`. Faça isso usando uma função auxiliar do SDK {admc-short-name}: `kubewarden_testing.BuildValidationRequest`.
...
Isso não acontece quando a política é avaliada em {admc-short-name}, nesse contexto as declarações `logger` emitem eventos OpenTelemetry em vez disso.
====
