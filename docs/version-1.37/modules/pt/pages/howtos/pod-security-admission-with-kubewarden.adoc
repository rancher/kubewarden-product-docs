...
= Usando o Pod Security Admission com {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-06-18
:page-revdate: {revdate}
:description: Configure o Pod Security Admission em um cluster Kubernetes com {admc-name} para proteger os pods.
...
{admc-short-name} pode ser usado para *integrar* um perfil de PSA para evitar essas limitações.

[NOTE]
====

Você pode usar {admc-short-name} para substituir a configuração antiga do PSP, conforme mostrado em xref:howtos/psp-migration.adoc[migração do PSP].
No entanto, o objetivo deste artigo é mostrar como {admc-short-name} pode complementar o novo PSA.
====
...
É aqui que {admc-short-name} pode ajudar.
...
Você precisa da pilha {admc-short-name} no cluster Kubernetes para este exemplo.
...
Agora a implantação é alterada pela política de {admc-short-name}, que permite que o Pod seja iniciado.
...
A integração {admc-short-name} pode fazer mais neste cenário.
Ela pode verificar o valor do `runAsUser` fornecido.

Este recurso é rejeitado pela política {admc-short-name} mencionada anteriormente:
...
Usar {admc-short-name} junto com o PSA ajuda a preencher essa lacuna.
