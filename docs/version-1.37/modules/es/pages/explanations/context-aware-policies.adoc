...
:keywords: ["{admc-name}", "kubewarden", "política consciente del contexto", "clusteradmissionpolicy", "admissionpolicy", "admissionrequest", "administrador del clúster"]
...
[NOTE]
====
Las políticas conscientes del contexto solo están disponibles en versiones de {admc-name} ≥ v1.6.0.
====
...
Por defecto, {admc-short-name} bloquea todos los recursos del clúster. Un administrador {admc-name} define qué recursos de Kubernetes cada política consciente del contexto puede leer. `contextAwareResources` para hacer esto.
...
Los autores de políticas proporcionan listas de recursos de Kubernetes para su política consciente del contexto. Los autores de políticas hacen esto anotando la política. Los administradores de {admc-name} ven los metadatos de la política utilizando el comando `kwctl inspect`. Pueden obtener una lista de recursos a los que la política necesita acceso. Un administrador utiliza esta lista para completar la definición de `ClusterAdmissionPolicy`.

[WARNING]
====

Para prevenir el abuso del sistema, los administradores de {admc-name} deben revisar los recursos a los que accede la política.
