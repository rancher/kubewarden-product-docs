...
Esta guía te muestra cómo habilitar TLS mutuo (mTLS) para webhooks de la pila {admc-name} al usar https://k3s.io/[K3s] como tu distribución de Kubernetes.
...
Antes de instalar K3s, necesitas crear una autoridad certificadora (CA) y un certificado de cliente. Se utiliza para asegurar la comunicación entre los webhooks de {admc-short-name} y el servidor API de Kubernetes.
...
El certificado de la CA raíz, que emitió el certificado del cliente de Kubernetes, debe estar disponible para el {admc-short-name} stack.
...
Instala el {admc-short-name} stack como se describe en la guía xref:/quick-start.adoc[quickstart]. Sigue todos los pasos, pero al instalar el gráfico Helm `kubewarden-controller`, asegúrate de habilitar los siguientes valores:
...
Una vez que este comando finalice, mTLS asegura la instalación del {admc-short-name} stack y sus webhooks.