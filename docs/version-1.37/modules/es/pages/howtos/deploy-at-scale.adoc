...
= {admc-name} en un entorno a gran escala
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-06-18
:page-revdate: {revdate}
:description: Configura {admc-name}s con {admc-name} para garantizar alta disponibilidad y rendimiento en entornos a gran escala exigentes.
:keywords: [kubewarden, "{admc-name}", kubernetes, policyservers, producción]
...
Esta sección detalla un despliegue del mundo real de {admc-name} en un entorno a gran escala exigente. Ilustra cómo configurar {admc-short-name} para alta disponibilidad y rendimiento y qué esperar bajo una carga pesada.

[IMPORTANT]
====
Si quieres ver más consejos sobre cómo ejecutar {admc-short-name} en producción, consulta la documentación de xref:/howtos/production-deployments.adoc[Despliegues en producción].
====
...
== {admc-short-name} Configuración

Para satisfacer las demandas de este entorno, {admc-short-name} está configurado con un enfoque en el aislamiento de cargas de trabajo y alta disponibilidad.
