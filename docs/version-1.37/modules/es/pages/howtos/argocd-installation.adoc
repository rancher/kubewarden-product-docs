...
Si estás utilizando ArgoCD para gestionar tus recursos de Kubernetes, puede que desees instalar {admc-name} utilizando ArgoCD. Esto es posible, pero hay ciertas consideraciones. Desde la versión {admc-short-name} v1.17.0, la dependencia cert-manager ha sido eliminada. Hay un nuevo reconciliador de certificados que renueva automáticamente los certificados. Esto significa que el controlador {admc-short-name} se encarga de crear y renovar los certificados por ti. Esto incluye tanto la CA raíz como todos los certificados de webhook.
...
En el ejemplo, el aspecto clave es la configuración del `ignoreDifferences` para la aplicación del controlador {admc-short-name}. El resto del contenido proporciona contexto sobre cómo configurar la aplicación ArgoCD y ofrecer un ejemplo completo.
