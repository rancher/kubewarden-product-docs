...
Las invocaciones siguientes son rápidas porque {admc-name} utiliza la función de caché de Wasmtime.
La primera ejecución puede tardar unos 20 segundos con `kwctl`, las ejecuciones posteriores, de 1 a 2 segundos.
Así que, {admc-short-name} Policy Server tiene un tiempo de inicio más lento, pero luego los tiempos de evaluación de directivas no suelen verse afectados por el uso de objetos de Kubernetes.
====
...
. Crea un objeto `payload`. Haz esto utilizando una función auxiliar del SDK de {admc-short-name}: `kubewarden_testing.BuildValidationRequest`.
...
Esto no sucede cuando la directiva se evalúa en {admc-short-name}, en ese contexto las declaraciones de `logger` emiten eventos de OpenTelemetry en su lugar.
====
