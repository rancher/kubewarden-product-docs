...
:keywords: "{admc-name}", kubewarden, kubernetes, escribiendo políticas, typescript
:doc-type: tutorial
:doc-topic: {admc-name}, kubewarden, escribiendo-políticas, typescript, lógica-de-validación
...
Escribir en STDOUT rompe las políticas. En su lugar, utiliza STDERR para el registro o la instalación de registro proporcionada por el SDK de {admc-name}. La salida de la directiva a STDOUT solo debe contener la respuesta de validación.
====
...
Puedes acceder a los datos de este objeto JSON a través de las funciones auxiliares del SDK de {admc-short-name}.
...
El {admc-short-name} complemento de Javy proporciona la `policyAction()` función que indica si la directiva debe validar un recurso o validar su configuración.