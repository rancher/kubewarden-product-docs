...
:description: 配置{admc-name}与{admc-name}以确保在要求苛刻的大规模环境中实现高可用性和性能。
...
本节详细介绍了{admc-name}在要求苛刻的大规模环境中的实际部署。它说明了如何配置{admc-short-name}以实现高可用性和性能，以及在高负载下的预期。

[IMPORTANT]
====
如果您想了解更多关于如何在生产中运行{admc-short-name}的提示，请查看xref:/howtos/production-deployments.adoc[生产部署]文档。
====
...
为了满足该环境的需求，{admc-short-name} 被配置为专注于工作负载隔离和高可用性。
