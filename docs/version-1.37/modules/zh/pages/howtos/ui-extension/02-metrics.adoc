...
:description: 通过与{admc-name}和 Prometheus 集成，为Rancher环境配置监控。
...
Rancher有一个用于监控的集群工具，使用Grafana和Prometheus。您可以使用此工具，将其与{admc-name}集成，以查看策略服务器的整体指标或特定策略的指标。
...
您应该能够在每个相应资源的详细页面上查看策略服务器或任何给定策略的指标。您还可以在Grafana UI中查看{admc-short-name}仪表板，或在Prometheus UI中查看事件。
