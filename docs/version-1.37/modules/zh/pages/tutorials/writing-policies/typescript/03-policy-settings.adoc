...
:description: 通过 {admc-name} 策略在 src/types.ts 和 src/index.ts 文件中定义和实现有效的策略设置结构。
:keywords: "{admc-name}", kubewarden, kubernetes, 定义策略设置, TypeScript
:doc-type: 教程
:doc-topic: {admc-name}, kubewarden, 编写策略, typescript, 定义策略设置
...
写入 STDOUT 会破坏策略。相反，使用 STDERR 进行日志记录或使用 {admc-name} SDK 提供的日志记录功能。策略输出到 STDOUT 只能包含验证响应。
====
