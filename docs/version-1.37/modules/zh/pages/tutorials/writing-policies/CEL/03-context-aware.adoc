...
为此，我们可以使用策略中包含的https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name}的CEL扩展库（用于主机能力）]。
...
现在，我们需要编写CEL代码，以获取集群中现有的Ingress资源。为此，我们使用[{admc-short-name}的CEL扩展库](https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities)。
