...
:doctype: book
:keywords: ["{admc-name}", "Kubewarden", "策略组", "集群准入策略组", "准入策略组"]
:sidebar_label: 策略组
:sidebar_position: 21
...
策略字段是普通策略的映射。{admc-name} 通过策略组调用策略，以确定是否接受或拒绝正在评估的资源。这些策略的定义是普通 {admc-short-name} 策略的简化版本，仅包含 `module`、`settings` 和 `contextAwareResources` 属性。这些元素对于策略在策略组中正常工作是必要的。
