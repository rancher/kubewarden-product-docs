...
`ghcr.io/kubewarden/adm-controller/policy-server:v1.36.0`). When you restore
these resources after the migration, the explicit field overrides the
{admc-name} chart default, so the PolicyServer pods keep pulling the community
image instead of the {admc-name} one
(`dp.apps.rancher.io/containers/kubewarden-policy-server:1.37.0`). You must
update `spec.image` on every restored PolicyServer after applying the backup.
See <<step-restore>>.
...
Once the controller is ready, restore the backed-up resources:
...
[WARNING]
====
The restored PolicyServer resources carry a `spec.image` field pinned to the
community policy-server image. You must update it to the {admc-name}
policy-server image on every restored PolicyServer, otherwise the PolicyServer
pods will keep pulling the community image from `ghcr.io` instead of the
{admc-name} image from `dp.apps.rancher.io`.

Patch all restored PolicyServers in a single loop:

[source,sh]
----
for ps in $(kubectl get policyservers -o name); do
  kubectl patch "$ps" --type=merge \
    -p '{"spec":{"image":"dp.apps.rancher.io/containers/kubewarden-policy-server:1.37.0"}}'
done
----

Alternatively, edit `spec.image` directly in `policyservers-backup.yaml` before
running `kubectl apply` above.
====
...
If any PolicyServer still shows a `ghcr.io/...` image, update it as described
in <<step-restore>>.
