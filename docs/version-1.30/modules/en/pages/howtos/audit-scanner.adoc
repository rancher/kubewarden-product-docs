...
Beginning with version `v1.7.0`, {project-name} has a new feature called "Audit Scanner".
A new component, called "audit-scanner", constantly checks the resources declared in the
cluster, flagging the ones that do not adhere with the deployed {project-name} policies.
...
The audit scanner component is available since {short-project-name} `v1.7.0`. Therefore,
...
`false` in the chart. This means that the {short-project-name} stack will not manage
