...
interested in installing {project-name} using ArgoCD. This is possible, but there
are certain considerations to consider. Since {short-project-name} v1.17.0, the
cert-manager dependency has been removed. There is a new certificate reconciler
that automatically renews the certificates. This means that the {short-project-name}
...
configuration for the {short-project-name} controller application. The rest of the
