...
The following ValidatingAdmissionPolicy is adapted from the https://kubernetes.io/docs/reference/access-authn-authz/validating-admission-policy/#creating-a-validatingadmissionpolicy[Kubernetes
docs].

This policy checks that the number of Replicas in Deployments is less than or
...
| 9   | `auditAnnotations` (not pictured) | `backgroundAudit`, annotations           | Use Kubewarden fields instead to set the policy usage in xref:explanations/audit-scanner/audit-scanner.adoc[Audit Scanner], and its category and severity for PolicyReports.
...
limit].
This is covered by Kubewarden's general xref:reference/policy-evaluation-timeout.adoc[policy timeout
protection].
