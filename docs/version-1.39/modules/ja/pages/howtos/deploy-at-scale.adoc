...
:description: {admc-name}を{admc-name}で構成し、要求の厳しい大規模環境において高可用性とパフォーマンスを確保します。
...
このセクションでは、要求の厳しい大規模環境における{admc-name}の実際のデプロイメントについて詳述します。高可用性とパフォーマンスのために{admc-short-name}を構成する方法と、重負荷時に何を期待するかを示しています。

[IMPORTANT]
====
本番環境で{admc-short-name}を運用するためのヒントをもっと見たい場合は、xref:/howtos/production-deployments.adoc[本番デプロイメント]のドキュメントをチェックしてください。
====
...
この環境の要求に応えるために、{admc-short-name}はワークロードの分離と高可用性に重点を置いて構成されています。
