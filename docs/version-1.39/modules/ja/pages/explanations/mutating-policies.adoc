...
:keywords: ["{admc-name}", "kubewarden", "ミューテーションポリシー", "kubernetes", "clusteradmissionpolicy", "admissionpolicy"]
...
システムの悪用を防ぐために、{admc-name}管理者はミューテーションポリシーをレビューするべきです。ミューテーションポリシーは、例えば、ワークロードを変更し、特権コンテナの作成を許可することがあります。
...
結論として、{admc-short-name}は廃止されたKubernetesポッドセキュリティポリシー（PSP）と同じ動作を再現することがわかります。
