...
:keywords: "{admc-name}", kubewarden, kubernetes, ポリシーの作成, typescript
:doc-type: チュートリアル
:doc-topic: {admc-name}, kubewarden, ポリシーの作成, typescript, 検証ロジック
...
STDOUTへの書き込みはポリシーを破ります。代わりに、ログ記録にはSTDERRを使用するか、{admc-name} SDKが提供するログ記録機能を使用してください。ポリシーのSTDOUTへの出力は、検証応答のみを含む必要があります。
====
...
このJSONオブジェクトデータには、{admc-short-name} SDKのヘルパー関数を通じてアクセスできます。
...
{admc-short-name} Javyプラグインは、リソースを検証するか、その設定を検証するかを示す`policyAction()`関数を提供します。