...
{admc-name} 实现对 https://www.sigstore.dev/[Sigstore] 项目的支持。
...
它们可以是容器镜像或更通用的 OCI 工件，例如 {admc-short-name} 策略。
当一个对象被签名时，它的签名作为由 Sigstore 创建的 OCI 对象的层存储。
需要检查容器签名的策略需要检查这些层，并需要拉取签名层以检查签名本身。

获取和操作这些 OCI 层需要在 WebAssembly 客户端（策略）之外进行。
因此，这由 WebAssembly 运行时 {admc-short-name} 的 `policy-server` 或 `kwctl` 完成。

不同语言的 {admc-short-name} 策略 SDK 负责处理此事。
它们提供容器镜像、{admc-short-name} 策略、Helm 图表和其他类型的 OCI 工件的验证功能。
这确保了验证的安全和经过测试的代码路径。
...
{admc-short-name} 团队 https://github.com/kubewarden/verify-image-signatures[提供一个验证器策略]，强制所有容器使用 Sigstore 签名。
