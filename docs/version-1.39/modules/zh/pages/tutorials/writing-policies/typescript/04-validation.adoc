...
:keywords: "{admc-name}", kubewarden, kubernetes, 编写策略, typescript
:doc-type: 教程
:doc-topic: {admc-name}, kubewarden, 编写策略, typescript, 验证逻辑
...
写入 STDOUT 会违反策略。相反，请使用 STDERR 进行日志记录或使用 {admc-name} SDK 提供的日志记录功能。策略输出到 STDOUT 只能包含验证响应。
====
...
您可以通过 {admc-short-name} SDK 的辅助函数访问此 JSON 对象数据。
...
{admc-short-name} Javy 插件提供 `policyAction()` 函数，指示策略是否应验证资源或验证其设置。