...
Le serveur de stratégies `default` déployé par les charts Helm {admc-name} a accès aux ressources Kubernetes suivantes :
...
| Rego
| ✅
| Depuis la version {admc-short-name} 1.9

| WASI
| ✅
| Depuis la version {admc-short-name} 1.10.0, uniquement pour le SDK Go
|===

== Contraintes

La priorité de {admc-short-name} est de réduire le nombre de requêtes contre le serveur API Kubernetes. {admc-short-name} considère deux contraintes :
...
Les stratégies de {admc-short-name} peuvent récupérer plusieurs ressources en même temps. Par exemple, elles peuvent effectuer une requête comme « récupérer tous les Pods définis dans l'espace de noms `default` qui ont l'étiquette `color` définie sur ``green`` ».
...
Le serveur de stratégies crée deux watches et duplique tous les Pods de la deuxième requête. Cette limitation sera supprimée dans les futures versions de {admc-short-name}.

=== Récupérer une ressource spécifique

Les stratégies de {admc-short-name} peuvent obtenir une ressource spécifique définie à l'intérieur du cluster.
...
Le comportement de requête directe ou mise en cache est configuré au niveau de chaque requête par l'auteur de la stratégie utilisant les SDK {admc-short-name}.
