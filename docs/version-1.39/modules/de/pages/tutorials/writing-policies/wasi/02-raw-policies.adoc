...
Sie müssen auch die `ValidationResponse` Struktur und die `MutateRequest` Funktion in `kw_sdk.go` aktualisieren, um die Kubernetes-spezifischen Typen zu entfernen und stattdessen {admc-short-name} Typen zu verwenden:
