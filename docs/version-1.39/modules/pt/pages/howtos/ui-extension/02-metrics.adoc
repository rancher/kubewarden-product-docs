...
:description: Configure o monitoramento para ambientes Rancher integrando com o {admc-name} e o Prometheus.
...
O Rancher possui uma Ferramenta de Cluster para monitoramento que utiliza Grafana e Prometheus. Você pode usar esta ferramenta, integrando-a com {admc-name}, para visualizar métricas gerais de um Servidor de Políticas ou métricas para uma política específica.
...
* Você precisa especificar o namespace de instalação do {admc-short-name}
...
Você deve ser capaz de visualizar as métricas para um Servidor de Políticas ou qualquer Política específica na página de detalhes de cada recurso respectivo. Você também pode visualizar os painéis {admc-short-name} dentro da interface do Grafana, ou os eventos da interface do Prometheus.
