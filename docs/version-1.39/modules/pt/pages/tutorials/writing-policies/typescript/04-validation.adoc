...
:keywords: "{admc-name}", kubewarden, kubernetes, escrevendo políticas, typescript
:doc-type: tutorial
:doc-topic: {admc-name}, kubewarden, escrevendo-políticas, typescript, lógica-de-validação
...
Escrever no STDOUT quebra as políticas. Em vez disso, use STDERR para log ou a funcionalidade de log fornecida pelo {admc-name} SDK. A saída da política para o STDOUT deve conter apenas a resposta de validação.
====
...
Você pode acessar os dados deste objeto JSON através das funções auxiliares do {admc-short-name} SDK.
...
O {admc-short-name} Javy plug-in fornece a função `policyAction()` indicando se a política deve validar um recurso ou validar suas configurações.