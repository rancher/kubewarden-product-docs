...
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "grupos de políticas", "grupo de política de admissão de cluster", "grupo de política de admissão"]
...
O campo de políticas é um mapa de políticas comuns. {admc-name} chama políticas pelo grupo de políticas, para determinar se aceita ou rejeita o recurso em avaliação. As definições dessas políticas são uma versão simplificada das políticas comuns {admc-short-name}, contendo apenas os atributos `module`, `settings` e `contextAwareResources`. Esses elementos são necessários para que as políticas funcionem dentro de um grupo de políticas.
