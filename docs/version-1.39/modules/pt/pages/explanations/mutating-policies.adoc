...
:keywords: ["{admc-name}", "kubewarden", "políticas mutantes", "kubernetes", "clusteradmissionpolicy", "admissionpolicy"]
...
Para prevenir abusos do sistema, os administradores {admc-name} devem revisar as políticas mutantes. Políticas mutantes poderiam, por exemplo, modificar uma carga de trabalho, de modo que permita a criação de contêineres privilegiados.
...
Em conclusão, você pode ver que {admc-short-name} replica o mesmo comportamento das Políticas de Segurança de Pod do Kubernetes (PSP) que foram descontinuadas.
