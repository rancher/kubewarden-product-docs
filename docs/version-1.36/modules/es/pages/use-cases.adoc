...
Los operadores pueden recibir información de las políticas y de la pila {admc-short-name} consumiendo registros e información de OpenTelemetry para métricas y trazado.
...
Como operador, despliego {admc-short-name} como en el Caso A para un conjunto de políticas de mi elección. Esto me proporciona una base segura en el clúster, que otros usuarios no pueden evadir.
...
== Caso D: Como integrador de sistemas, quiero reutilizar {admc-short-name} como parte de mi solución de seguridad y cumplimiento.

Como integrador de sistemas, quiero reutilizar {admc-short-name}, y posiblemente reutilizar otras soluciones incluyéndolas en {admc-short-name}.

Como integrador de sistemas, puedo reutilizar partes de {admc-short-name}. Por ejemplo, el `policy-server`, para regular recursos, internos o externos al clúster de Kubernetes a través de la función xref:/howtos/raw-policies.adoc["políticas en bruto"].
...
{admc-short-name} no pretende:

- Reemplazar las características de seguridad integradas de Kubernetes, sino complementarlas:
  - {admc-short-name} proporciona migración desde PSPs.
  - Puedes reutilizar ValidatingAdmissionPolicies y políticas CEL con el {admc-short-name} de `cel-policy`.
  - Las políticas de {admc-short-name} pueden ser de mutación, mientras que la Admisión de Seguridad de Pods no puede.
  - Las políticas de {admc-short-name} se benefician de las características de la pila {admc-short-name} (escáner de auditoría, telemetría, gestión de CRD).
