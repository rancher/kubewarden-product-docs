...
:title: kwctl CLI
:description: Explora la herramienta de línea de comandos \`kwctl` para gestionar directivas de {admc-name} y realizar tareas como cargar directivas.
...
Herramienta para gestionar directivas de {admc-name}

*Uso:* `kwctl [OPTIONS] <COMMAND>`

==== Subcomandos

* `annotate` — Añadir metadatos de {admc-short-name} a un módulo WebAssembly
* `bench` — Realiza pruebas de rendimiento de una directiva de {admc-short-name}
...
* `inspect` — Inspecciona la directiva de {admc-short-name}
* `load` — Carga políticas desde un archivo tar.gz
* `policies` — Lista todas las políticas descargadas
* `pull` — Extrae una directiva de {admc-short-name} desde una URI dada
* `push` — Envía una directiva {admc-short-name} a un registro OCI
* `rm` — Elimina una directiva {admc-short-name} de la tienda
* `run` — Ejecuta una directiva {admc-short-name} desde una URI dada
* `save` — Guarda directivas en un archivo tar.gz
* `scaffold` — Crea un recurso o archivo de configuración de Kubernetes
* `verify` — Verifica una directiva {admc-short-name} desde una URI dada usando Sigstore
...
Añadir {admc-short-name} metadatos a un módulo WebAssembly
...
Evalúa una directiva {admc-short-name}.
...
- Archivo YAML local: p. ej., `file://home/tux/cluster-admission-policy.yaml` (contiene declaraciones de Recursos Personalizados {admc-short-name} como `ClusterAdmissionPolicy`, `AdmissionPolicy`, etc.)

Comportamiento por defecto:
Si se omite el esquema, se asume `file://`, enraizado en el directorio actual.

Notas sobre Recursos Personalizados {admc-short-name}:
...
- La bandera `--raw` no se puede usar, ya que los Recursos Personalizados de {admc-short-name} no soportan directivas `raw`.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — URI de la directiva, prefijo SHA o archivo YAML que contiene recursos de directiva {admc-short-name}. Esquemas soportados: registry://, https://, file://. Si se omite el esquema, se asume file://, enraizado en el directorio actual.
...
Inspeccionar la directiva {admc-short-name}
...
Extrae una directiva {admc-short-name} desde una URI dada
...
* `--github-owner <VALUE>` — Propietario de GitHub esperado en los certificados generados en las canalizaciones de CD
* `--github-repo <VALUE>` — Repositorio de GitHub esperado en los certificados generados en las canalizaciones de CD
* `-o`, `--output-path <PATH>` — Archivo de salida. Si no se proporciona, se descargará en la tienda {admc-short-name}
* `--sigstore-trust-config <PATH>` — Archivo en formato JSON que se ajusta al mensaje ClientTrustConfig en las especificaciones protobuf de Sigstore. Este archivo configura todo el estado de la instancia de Sigstore, incluyendo las URIs utilizadas para acceder a los servicios de CA y transparencia de artefactos, así como la raíz de confianza criptográfica en sí.
* `--sources-path <PATH>` — Archivo YAML que contiene información de origen (https, hosts inseguros de registro, CA personalizadas...)
* `-a`, `--verification-annotation <KEY=VALUE>` — Anotación en formato clave=valor. Puede repetirse múltiples veces
* `--verification-config-path <PATH>` — Archivo YAML que contiene información de configuración de verificación (firmas, claves públicas...)
...
Envía una directiva {admc-short-name} a un registro OCI
...
Elimina una directiva {admc-short-name} del almacén
...
Ejecuta una o más {admc-short-name} directivas localmente.

La directiva se puede especificar de las siguientes maneras:
- URI: p. ej., `registry://ghcr.io/kubewarden/policies/psp-policy:latest` o `https://example.com/kubewarden/policies/main/psp-policy/psp-policy.wasm`
- Prefijo SHA: p. ej., `c3b80a10f9c3` (requiere que la directiva ya esté descargada)
- Archivo WASM local: p. ej., `file://home/tux/new-policy/psp-policy.wasm`
- Archivo YAML local: p. ej., `file://home/tux/cluster-admission-policy.yaml` (contiene declaraciones de {admc-short-name} Recursos Personalizados como `ClusterAdmissionPolicy`, `AdmissionPolicy`, etc.)
...
* `vap` — Convierte un `ValidatingAdmissionPolicy` de Kubernetes en un {admc-short-name} `ClusterAdmissionPolicy`
...
* `-o`, `--operation <TYPE>` — {admc-short-name} Tipo de Recurso Personalizado +
...
* `-t`, `--type <VALUE>` — {admc-short-name} Tipo de Recurso Personalizado +
...
Convertir un `ValidatingAdmissionPolicy` de Kubernetes en un {admc-short-name} `ClusterAdmissionPolicy`
...
Verifica una directiva de {admc-short-name} desde una URI dada utilizando Sigstore
