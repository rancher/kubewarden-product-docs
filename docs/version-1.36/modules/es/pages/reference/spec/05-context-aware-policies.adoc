...
El `default` servidor de políticas desplegado por {admc-name} gráficos de Helm tiene acceso a los siguientes recursos de Kubernetes:
...
| Rego
| ✅
| Desde la versión {admc-short-name} 1.9

| WASI
| ✅
| Desde la versión {admc-short-name} 1.10.0, solo para el SDK de Go
|===

== Restricciones

La prioridad de {admc-short-name} es reducir el número de consultas contra el servidor API de Kubernetes. {admc-short-name} considera dos restricciones:
...
Las políticas {admc-short-name} pueden obtener múltiples recursos al mismo tiempo. Por ejemplo, pueden hacer una consulta como "obtener todos los Pods definidos dentro del espacio de nombres `default` que tienen la etiqueta `color` establecida en ``green``."
...
El Servidor de Políticas crea dos watches y duplica todos los Pods de la segunda consulta. Esta limitación se eliminará en futuras versiones de {admc-short-name}.

=== Obteniendo un recurso específico

Las {admc-short-name} políticas pueden obtener un recurso específico definido dentro del clúster.
...
El comportamiento de consulta directa o en caché se configura a nivel de cada consulta por el autor de la política utilizando los SDKs {admc-short-name}.
