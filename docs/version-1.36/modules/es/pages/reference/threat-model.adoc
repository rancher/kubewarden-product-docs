...
El https://github.com/kubernetes/community/tree/master/sig-security[Grupo de Interés Especial (SIG) de Seguridad de Kubernetes] ha definido un Modelo de Amenaza de Control de Admisión para Kubernetes. El equipo {admc-name} evalúa continuamente {admc-short-name} en relación con este modelo de amenaza y trabaja para proporcionar configuraciones seguras. Se recomienda que los administradores de {admc-short-name} lean y comprendan el modelo de amenaza, y lo utilicen para idear su propio modelo de amenaza específico para las circunstancias según sea necesario.
...
Este es el comportamiento por defecto de {admc-short-name}.

Fallar cerrado significa que si, por cualquier motivo, {admc-short-name} deja de responder o falla, el servidor API rechaza la solicitud por defecto.
Esto es incluso si la solicitud es normalmente aceptada por {admc-short-name}.
...
La mayor parte de RBAC no está dentro del alcance de la discusión actual. Sin embargo, lo siguiente llegará a su debido tiempo, para ayudar a los usuarios de {admc-short-name}:
...
Dado que el webhook utiliza cifrado TLS para todo el tráfico, {admc-short-name} es seguro.
...
La mayor parte del RBAC está fuera del alcance respecto a esta decisión. Sin embargo, el equipo {admc-short-name} tiene como objetivo:
...
* Configura el clúster con autenticación mTLS para los Webhooks y habilita la función mTLS en la pila {admc-short-name}. Alternativamente, configura mTLS utilizando un CNI que soporte Políticas de Red.

* Por defecto, las directivas de {admc-short-name} no tienen acceso a la red y se ejecutan en un entorno restrictivo, controlando estrictamente el acceso externo a los Webhooks.

== Amenazas de {admc-short-name}

=== Amenaza {admc-short-name} 1 - Inicio de confianza para el controlador de admisión.

==== Escenario

Suponiendo un clúster de Kubernetes confiable pero nuevo, un atacante puede comprometer la pila {admc-short-name} antes de su ampliación y de la aplicación de cualquiera de las directivas de seguridad.
 ...
 ** cualquiera de las dependencias de {admc-short-name}
 ** cualquiera de las dependencias opcionales (Grafana, Prometheus y otras)
* comprometiendo la carga útil de los gráficos de Helm

==== Mitigación

. {admc-short-name} proporciona un Software Bill Of Materials, que enumera todas las imágenes necesarias. Esto ayuda con Zero-Trust. El administrador de Kubernetes debe verificar las imágenes de {admc-short-name}, las imágenes de sus dependencias y los gráficos fuera del clúster de Kubernetes, en un entorno de confianza. Puedes hacer esto con `cosign`, por ejemplo. Por cierto, esta es parte de la implementación necesaria para instalaciones en un entorno aislado.
. Utiliza gráficos de Helm firmados y resúmenes verificados en lugar de etiquetas para las imágenes de {admc-short-name} en esos gráficos de Helm. Sin embargo, esto no asegura las dependencias.