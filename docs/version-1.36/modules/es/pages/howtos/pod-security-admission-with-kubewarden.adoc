...
= Usando la Admisión de Seguridad de Pods con {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Configura la Admisión de Seguridad de Pods en un clúster de Kubernetes con {admc-name} para asegurar los pods.
...
{admc-short-name} se puede usar para *integrar* un perfil de PSA para evitar estas limitaciones.

[NOTE]
====

Podrías usar {admc-short-name} para reemplazar la antigua configuración de PSP como se muestra en xref:howtos/psp-migration.adoc[migración de PSP].
Sin embargo, el objetivo de este artículo es mostrar cómo {admc-short-name} puede complementar el nuevo PSA.
====
...
Aquí es donde {admc-short-name} puede ayudar.
...
Necesitas la pila {admc-short-name} en el clúster de Kubernetes para este ejemplo.
...
Ahora la ampliación está mutada por la directiva de {admc-short-name} que permite que el Pod se inicie.
...
La integración de {admc-short-name} puede hacer más en este escenario.
Puede verificar el valor del `runAsUser` proporcionado.

Este recurso es rechazado por la directiva de {admc-short-name} de antes:
...
Usar {admc-short-name} junto con PSA ayuda a llenar este vacío.
