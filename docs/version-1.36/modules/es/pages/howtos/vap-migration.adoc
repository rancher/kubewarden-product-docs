...
:description: Migra un ValidatingAdmissionPolicy a {admc-name} a través de los pasos de migración del comando kwctl para una transición de política fluida.
...
{admc-name} proporciona una política CEL que es capaz de ejecutar políticas VAP de Kubernetes sin ninguna modificación.
Puedes leer más sobre la política CEL en xref:../tutorials/writing-policies/CEL/01-intro-cel.adoc[esta sección] de la documentación de {admc-short-name}.
xref:tutorials/writing-policies/CEL/01-intro-cel.adoc#_benefits_of_kubewardens_cel_policy_in_comparison_with_validatingadmissionpolicies[Este párrafo] explica los beneficios de ejecutar políticas VAP utilizando {admc-short-name}.

Esta guía explica cómo usar la herramienta `kwctl` para migrar una política VAP a {admc-short-name}.
...
Puedes migrar la política a {admc-short-name} siguiendo estos pasos:
