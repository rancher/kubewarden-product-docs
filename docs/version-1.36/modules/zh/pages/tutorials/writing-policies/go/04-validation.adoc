...
后续调用很快，因为 {admc-name} 使用了 Wasmtime 的缓存功能。
第一次执行可能需要大约 20 秒 `kwctl`，而后续执行需要 1 到 2 秒。
因此，{admc-short-name} 策略服务器的启动时间较慢，但随后策略评估时间通常不受使用 Kubernetes 对象的影响。
====
...
. 创建一个 `payload` 对象。使用 {admc-short-name} SDK 的辅助函数：`kubewarden_testing.BuildValidationRequest` 来完成此操作。
...
当策略在 {admc-short-name} 中评估时，这不会发生，在该上下文中，`logger` 语句会发出 OpenTelemetry 事件。
====
