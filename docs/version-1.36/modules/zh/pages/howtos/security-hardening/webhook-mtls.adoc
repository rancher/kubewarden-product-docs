...
本指南向您展示如何在使用 https://k3s.io/[K3s] 作为 Kubernetes 发行版时，为 {admc-name} 堆栈 Webhook 启用双向传输层安全性 (mTLS)。

有关如何加强 Webhook 的更多信息，请参阅 xref:/reference/security-hardening/webhooks-hardening.adoc[参考页面]。

== 先决条件

在安装 K3s 之前，您需要创建一个证书颁发机构 (CA) 和一个客户端证书。您用来保护 {admc-short-name} Webhook 与 Kubernetes API 服务器之间的通信。
...
颁发 Kubernetes 客户端证书的根 CA 证书需要在 {admc-short-name} 堆栈中可用。
...
按照 xref:/quick-start.adoc[快速入门指南] 中描述的方式安装 {admc-short-name} 堆栈。遵循所有步骤，但在安装 `kubewarden-controller` Helm 图表时，请确保启用以下值：
...
一旦此命令完成，mTLS 将确保 {admc-short-name} 堆栈及其 Webhook 的安装安全。