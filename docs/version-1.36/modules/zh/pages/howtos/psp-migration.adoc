...
{admc-short-name} 拥有单独的策略，以实现与单体 PSP 配置相同的目标。每个 {admc-short-name} 策略定义在 PSP 的规范中作为不同的配置部分。PSP 配置字段与其各自的 {admc-short-name} 策略的映射关系见下方 <<_mapping_kubewarden_policies_to_psp_fields,映射表>>。
...
{admc-short-name}团队开发了一个用于PSP迁移的脚本。它使用来自https://github.com/appvia/psp-migration[AppVia]的迁移工具。AppVia 工具读取 PSP YAML 配置。它会生成相应的策略。它为 {admc-short-name} 和其他策略引擎执行此操作。
