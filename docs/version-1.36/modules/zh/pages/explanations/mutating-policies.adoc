...
:keywords: ["{admc-name}", "kubewarden", "变更策略", "kubernetes", "clusteradmissionpolicy", "admissionpolicy"]
:sidebar_label: 变更策略
:sidebar_position: 10
...
为了防止系统滥用，{admc-name}管理员应审查变更策略。变更策略可能会修改工作负载，从而允许特权容器的创建。
...
总之，您可以看到{admc-short-name}复制了已弃用的 Kubernetes Pod 安全策略（PSP）的相同行为。
