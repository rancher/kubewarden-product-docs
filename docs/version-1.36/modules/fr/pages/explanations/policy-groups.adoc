...
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "groupes de stratégies", "groupe de stratégies d'admission de cluster", "groupe de stratégies d'admission"]
...
Le champ des stratégies est un mappage de stratégies ordinaires. {admc-name} appelle les stratégies par le groupe de stratégies, pour déterminer s'il faut accepter ou rejeter la ressource en cours d'évaluation. Les définitions de ces stratégies sont une version simplifiée des stratégies ordinaires {admc-short-name}, contenant uniquement les attributs `module`, `settings` et `contextAwareResources`. Ces éléments sont nécessaires au bon fonctionnement des stratégies au sein d'un groupe de stratégies.
