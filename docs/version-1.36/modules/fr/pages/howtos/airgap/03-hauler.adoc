...
:description: Utilisez Hauler pour installer {admc-name} dans des environnements isolés physiquement.
:keywords: ["{admc-name}", Kubewarden, kubernetes, installation en isolation physique, Hauler]
:doc-persona: kubewarden-operator, kubewarden-integrator
:doc-type: howto
:doc-topic: manuel de l'opérateur, isolation physique, installation, Hauler

Ce guide vous montre comment installer {admc-name} dans des environnements isolés physiquement en utilisant link:https://docs.hauler.dev/docs/intro[Hauler]. Hauler est un outil qui aide les utilisateurs à exécuter des charges de travail dans des environnements isolés physiquement. Il déplace les ressources nécessaires pour les applis dans ces environnements isolés physiquement.

{admc-short-name} fournit un fichier manifeste avec toutes les ressources nécessaires pour exécuter {admc-short-name} dans un environnement privé. Cette documentation décrit comment vous pouvez l'utiliser. Référez-vous à la documentation de Hauler pour en savoir plus à son sujet.

Le flux de travail de base utilisant le manifeste de Hauler est :

. Obtenez-le à partir de la page de version de {admc-short-name}
. Chargez toutes les ressources nécessaires pour exécuter {admc-short-name} dans un magasin local
...
Téléchargez `hauler_manifest.yml` depuis le {admc-short-name} link:https://github.com/kubewarden/helm-charts/releases/[page de version].
...
== Générez le fichier tarball avec toutes les ressources {admc-short-name}.
...
{admc-short-name} container images support x86_64 and architecture ARM. Par conséquent, lorsque vous enregistrez la ressource dans le fichier, vous pouvez voir des messages d'avertissement comme celui-ci :
...
Maintenant que vous avez toutes les ressources {admc-short-name} dans `kubewarden-resources.tar.zst`, copiez-les dans votre environnement isolé physiquement et chargez-les dans le magasin Hauler là-bas :
...
Maintenant, toutes les ressources nécessaires pour installer {admc-short-name} sont dans le magasin Hauler de votre environnement isolé physiquement.
...
Cela démarre un registre à l'adresse `localhost:5000`. À partir de ce point, vous pouvez utiliser d'autres commandes comme Skopeo pour copier toutes les images de conteneurs, les modules de politique et les charts Helm utilisés par {admc-short-name} dans votre registre privé.

== Installez {admc-short-name}

Maintenant que votre registre privé a tout ce qu'il faut, vous pouvez installer {admc-short-name}. La différence par rapport à une installation standard de {admc-short-name} est que vous devez changer le registre dans les images de conteneurs et les politiques pour qu'il soit le registre privé. De plus, vous devez installer les charts Helm à partir des artefacts OCI.
...
Créez maintenant {admc-short-name} politiques dans votre cluster. Les politiques doivent être disponibles dans votre registre privé.
