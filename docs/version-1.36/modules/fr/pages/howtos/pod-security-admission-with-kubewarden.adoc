...
= Utiliser l'admission de sécurité des pods avec {admc-name}
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Configurer l'admission de sécurité des pods dans un cluster Kubernetes avec {admc-name} pour sécuriser les pods.
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "admission de sécurité des pods", "politique de sécurité des pods", "kubernetes"]
...
{admc-short-name} peut être utilisé pour *intégrer* un profil PSA afin d'éviter ces limitations.

[NOTE]
====

Vous pourriez utiliser {admc-short-name} pour remplacer l'ancienne configuration PSP comme indiqué dans xref:howtos/psp-migration.adoc[migration PSP].
Cependant, l'objectif de cet article est de montrer comment {admc-short-name} peut compléter le nouveau PSA.
====
...
C'est ici que {admc-short-name} peut aider.
...
Vous avez besoin de la pile {admc-short-name} dans le cluster Kubernetes pour cet exemple.
...
Maintenant, le déploiement est modifié par la stratégie de {admc-short-name} qui permet au Pod de démarrer.
...
L'intégration {admc-short-name} peut faire plus dans ce scénario.
Elle peut vérifier la valeur du `runAsUser` fourni.

Cette ressource est rejetée par la stratégie {admc-short-name} mentionnée précédemment :
...
Utiliser {admc-short-name} avec PSA aide à combler cette lacune.
