...
Si vous utilisez ArgoCD pour gérer vos ressources Kubernetes, vous souhaiterez peut-être installer {admc-name} en utilisant ArgoCD. C'est possible, mais il y a certaines considérations. Depuis la version {admc-short-name} v1.17.0, la dépendance cert-manager a été supprimée. Il y a un nouveau réconciliateur de certificats qui renouvelle automatiquement les certificats. Cela signifie que le contrôleur {admc-short-name} s'occupe de créer et de renouveler les certificats pour vous. Cela inclut à la fois le CA racine et tous les certificats de webhook.
...
Dans l'exemple, l'aspect clé est la configuration `ignoreDifferences` pour l'application du contrôleur {admc-short-name}. Le reste du contenu fournit un contexte sur la façon de configurer l'application ArgoCD et de fournir un exemple complet.
