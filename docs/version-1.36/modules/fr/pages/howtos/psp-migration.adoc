...
{admc-short-name} a des politiques séparées pour atteindre le même objectif qu'une configuration monolithique de PSP. Chaque définition de politique {admc-short-name} fonctionne comme une section de configuration différente dans la spécification d'un PSP. La correspondance des champs de configuration de PSP avec leurs politiques {admc-short-name} respectives se trouve dans le <<_mapping_kubewarden_policies_to_psp_fields,tableau de correspondance>> ci-dessous.
