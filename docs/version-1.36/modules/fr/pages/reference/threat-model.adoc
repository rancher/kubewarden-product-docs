...
Le https://github.com/kubernetes/community/tree/master/sig-security[Groupe d'Intérêt Spécial (SIG) Sécurité Kubernetes] a défini un Modèle de Menace de Contrôle d'Admission pour Kubernetes. L'équipe {admc-name} évalue en continu {admc-short-name} par rapport à ce modèle de menace et travaille à fournir des valeurs par défaut sécurisées. Il est recommandé que les administrateurs {admc-short-name} lisent et comprennent le modèle de menace, et l'utilisent pour élaborer leur propre modèle de menace spécifique à leur situation si nécessaire.
...
C'est le comportement par défaut de {admc-short-name}.

Échouer en mode fermé signifie que si, pour une raison quelconque, {admc-short-name} cesse de répondre ou plante, le serveur API rejette la demande par défaut.
C'est même si la demande est normalement acceptée par {admc-short-name}.
...
La plupart des RBAC ne relèvent pas du champ de discussion actuel. Cependant, ce qui suit arrivera en temps voulu, pour aider les utilisateurs {admc-short-name} :
...
Puisque le webhook utilise le chiffrement TLS pour tout le trafic, {admc-short-name} est en sécurité.
...
Configurez le cluster avec une authentification mTLS pour les Webhooks et activez la fonctionnalité mTLS dans la pile {admc-short-name}. Alternativement, configurez mTLS en utilisant un CNI qui prend en charge les politiques réseau. Référez-vous à xref:/reference/security-hardening/webhooks-hardening.adoc["Sécuriser les webhooks avec TLS mutuel"] pour plus d'informations.
...
La plupart des RBAC ne sont pas concernés par cette décision. Cependant, l'équipe {admc-short-name} vise à :
...
* Configurez le cluster avec l'authentification mTLS pour les Webhooks et activez la fonctionnalité mTLS dans la pile {admc-short-name}. Alternativement, configurez mTLS en utilisant un CNI qui prend en charge les politiques réseau.

* Par défaut, les politiques {admc-short-name} n'ont pas d'accès réseau et s'exécutent dans un environnement restrictif, contrôlant strictement l'accès externe sur les Webhooks.

== Menaces {admc-short-name}

=== Menace {admc-short-name} 1 - Amorçage de la confiance pour le contrôleur d'admission

==== Scénario

En supposant un cluster Kubernetes de confiance mais nouveau, un attaquant est capable de compromettre la pile {admc-short-name} avant le déploiement et l'application de l'une des politiques de sécurité.

Par exemple, par:

* utilisant des images non signées et malveillantes pour :
 ** {admc-short-name}-contrôleur
 ** serveur de stratégie
 ** n'importe lesquelles des {admc-short-name} dépendances
 ** toutes les dépendances optionnelles (Grafana, Prometheus, et autres)
* en compromettant le contenu des charts Helm

==== Mitigation

. {admc-short-name} fournit une liste des composants logiciels (SBOM), qui répertorie toutes les images nécessaires. Cela facilite le Zero Trust. L'administrateur Kubernetes doit vérifier {admc-short-name} les images, les images de ses dépendances et les charts en dehors du cluster Kubernetes, dans un environnement de confiance. Vous pouvez le faire avec `cosign`, par exemple. Incidemment, cela fait partie de la mise en œuvre nécessaire pour les installations isolées physiquement.
. Utilisez des charts Helm signés et des empreintes vérifiées au lieu de tags pour {admc-short-name} les images dans ces charts Helm. Cela ne sécurise cependant pas les dépendances.