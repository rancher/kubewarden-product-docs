...
:title: kwctl CLI
:description: Explorez l'outil de ligne de commande \`kwctl` pour gérer les stratégies {admc-name} et effectuer des tâches telles que le chargement de stratégies.
...
Outil pour gérer les stratégies {admc-name}

*Utilisation:* `kwctl [OPTIONS] <COMMAND>`

==== Sous-commandes

* `annotate` — Ajoute des métadonnées {admc-short-name} à un module WebAssembly
* `bench` — Évalue une stratégie {admc-short-name}
...
* `inspect` — Inspecte la stratégie {admc-short-name}
* `load` — Charge des stratégies à partir d'un fichier tar.gz
* `policies` — Liste toutes les stratégies téléchargées
* `pull` — Récupère une stratégie {admc-short-name} à partir d'une URI donnée
* `push` — Pousse une stratégie {admc-short-name} vers un registre OCI
* `rm` — Supprime une stratégie {admc-short-name} du magasin
* `run` — Exécute une stratégie {admc-short-name} à partir d'une URI donnée
...
* `verify` — Vérifie une stratégie {admc-short-name} à partir d'une URI donnée en utilisant Sigstore
...
Ajoute des métadonnées {admc-short-name} à un module WebAssembly
...
Évalue une stratégie {admc-short-name}.
...
- Fichier YAML local : par exemple, `file://home/tux/cluster-admission-policy.yaml` (contient des déclarations de {admc-short-name} Ressources Personnalisées comme `ClusterAdmissionPolicy`, `AdmissionPolicy`, etc.)

Comportement par défaut :
Si le schéma est omis, `file://` est supposé, enraciné dans le répertoire courant.

Remarques sur les Ressources Personnalisées {admc-short-name} : +
...
- L'indicateur `--raw` ne peut pas être utilisé, car les Ressources Personnalisées de {admc-short-name} ne prennent pas en charge les stratégies `raw`.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — URI de la stratégie, préfixe SHA ou fichier YAML contenant les ressources de stratégie {admc-short-name}. Schémas pris en charge : registry://, https://, file://. Si le schéma est omis, file:// est supposé, enraciné dans le répertoire courant.
...
Inspecter la stratégie {admc-short-name}
...
Récupère une stratégie {admc-short-name} d'une URI donnée
...
* `--github-owner <VALUE>` — Propriétaire GitHub attendu dans les certificats générés dans les pipelines CD
* `--github-repo <VALUE>` — Dépôt GitHub attendu dans les certificats générés dans les pipelines CD
* `-o`, `--output-path <PATH>` — Fichier de sortie. Si non fourni, sera téléchargé dans le magasin {admc-short-name}
* `--sigstore-trust-config <PATH>` — Fichier au format JSON conforme au message ClientTrustConfig dans les spécifications protobuf de Sigstore. Ce fichier configure l'état entier de l'instance Sigstore, y compris les URI utilisés pour accéder aux services de CA et de transparence des artefacts ainsi que la racine de confiance cryptographique elle-même
* `--sources-path <PATH>` — Fichier YAML contenant des informations source (https, hôtes de registre non sécurisés, CA personnalisées...)
* `-a`, `--verification-annotation <KEY=VALUE>` — Annotation au format clé=valeur. Peut être répété plusieurs fois
* `--verification-config-path <PATH>` — Fichier YAML contenant des informations de configuration de vérification (signatures, clés publiques...)
...
Pousse une stratégie {admc-short-name} vers un registre OCI
...
Supprime une stratégie {admc-short-name} du magasin
...
Exécute une ou plusieurs stratégies {admc-short-name} localement.
...
* `<URI_OR_SHA_PREFIX_OR_YAML_FILE>` — URI de la stratégie, préfixe SHA ou fichier YAML contenant les ressources de stratégie {admc-short-name}. Schémas pris en charge : registry://, https://, file://. Si le schéma est omis, file:// est supposé, ancré dans le répertoire courant.
...
* `vap` — Convertir un `ValidatingAdmissionPolicy` Kubernetes en un {admc-short-name} `ClusterAdmissionPolicy`
...
* `-o`, `--operation <TYPE>` — Type de ressource personnalisée {admc-short-name} +
...
* `-t`, `--type <VALUE>` — {admc-short-name} type de ressource personnalisée +
...
Convertir Kubernetes `ValidatingAdmissionPolicy` en un {admc-short-name} `ClusterAdmissionPolicy`
...
Vérifiez une stratégie {admc-short-name} à partir d'une URI donnée en utilisant Sigstore
