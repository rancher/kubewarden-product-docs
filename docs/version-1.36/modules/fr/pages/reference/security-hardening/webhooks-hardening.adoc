...
= Renforcement de la sécurité des {admc-short-name} webhooks
...
La pile {admc-name} utilise des webhooks pour appliquer des stratégies dans un cluster Kubernetes.
Chaque instance `PolicyServer` expose un webhook que le serveur API Kubernetes appelle pour valider et modifier les ressources. De plus, le `kubewarden-controller` expose des webhooks pour valider et modifier les ressources personnalisées fournies par le projet {admc-short-name}.
...
Cette stratégie réseau utilise des sélecteurs d'étiquettes introduits dans {admc-short-name} 1.23.0. Si vous utilisez une version antérieure, mettez à jour les étiquettes dans la stratégie pour qu'elles correspondent à votre déploiement.
...
Les webhooks exposés par la pile {admc-short-name} ne devraient accepter que les requêtes du serveur API Kubernetes ou du composant scanner d'audit. Par défaut, ces webhooks ne nécessitent pas que les clients s'authentifient auprès d'eux. Ils acceptent toute requête.
...
. Créez le fichier `kubeconfig` auquel se réfèrent les plug-ins d'admission. {admc-short-name} ne prend en charge que l'authentification par certificat client, donc générez une paire de clés TLS et configurez le kubeconfig pour utiliser soit le certificat client et la clé client, soit les données du certificat client et les données de la clé client.
+
Par exemple :
+
[source,yaml]
...
. Rendez le certificat de l'autorité de certification racine qui a délivré le certificat client du serveur API disponible pour la pile {admc-short-name}.
+
...
----
+
Le contrôleur {admc-short-name} crée un certificat client à utiliser par le composant scanner d'audit. Le certificat est automatiquement renouvelé par le contrôleur lorsque cela est nécessaire.
