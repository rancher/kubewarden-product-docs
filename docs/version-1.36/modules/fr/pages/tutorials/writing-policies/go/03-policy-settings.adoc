...
Une stratégie {admc-short-name} expose deux fonctions différentes qui reçoivent les paramètres de stratégie en entrée :
...
* `validate_settings` : Appelez cette fonction lorsque la politique est chargée pour la première fois par {admc-short-name}.
...
Toutes les politiques {admc-short-name} doivent mettre en œuvre la validation des paramètres.
...
Vous pouvez voir que la fonction tire parti des fonctions utilitaires fournies par le SDK de https://github.com/kubewarden/policy-sdk-go[{admc-short-name}.]
