...
:description: Définissez et implémentez des structures de paramètres de stratégie efficaces à travers les stratégies {admc-name} dans les fichiers src/types.ts et src/index.ts.
:keywords: "{admc-name}", kubewarden, kubernetes, définition des paramètres de stratégie, TypeScript
:doc-type: didacticiel
:doc-topic: {admc-name}, kubewarden, rédaction-des-stratégies, typescript, définition-des-paramètres-de-stratégie
...
Écrire sur STDOUT enfreint les stratégies. Utilisez plutôt STDERR pour la journalisation ou le service de journalisation fourni par le {admc-name} SDK. La sortie de la stratégie sur STDOUT ne doit contenir que la réponse de validation.
====
...
Les stratégies {admc-short-name} utilisent deux fonctions qui gèrent les paramètres :
