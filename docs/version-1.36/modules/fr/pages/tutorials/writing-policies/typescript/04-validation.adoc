...
:keywords: "{admc-name}", kubewarden, kubernetes, rédaction de stratégies, typescript
:doc-type: didacticiel
:doc-topic: {admc-name}, kubewarden, rédaction-de-stratégies, typescript, logique-de-validation
...
Écrire sur STDOUT enfreint les stratégies. Utilisez plutôt STDERR pour la journalisation ou le service de journalisation fourni par le {admc-name} SDK. La sortie de la stratégie vers STDOUT ne doit contenir que la réponse de validation.
====
...
Vous pouvez accéder aux données de cet objet JSON via les fonctions auxiliaires du {admc-short-name} SDK.
...
Le {admc-short-name} plug-in Javy fournit la fonction `policyAction()` indiquant si la stratégie doit valider une ressource ou valider ses paramètres.