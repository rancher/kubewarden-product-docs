...
:description: Definieren und Implementieren effektiver Strukturen für Richtlinieneinstellungen durch {admc-name} Richtlinien in den Dateien src/types.ts und src/index.ts.
:keywords: "{admc-name}", kubewarden, kubernetes, Festlegung von Richtlinieneinstellungen, TypeScript
:doc-type: Anleitung
:doc-topic: {admc-name}, kubewarden, Richtlinien schreiben, typescript, Festlegung von Richtlinieneinstellungen
...
Das Schreiben in STDOUT bricht die Richtlinien. Verwenden Sie stattdessen STDERR für das Protokollieren oder die Protokollierungsfunktionalität, die vom {admc-name} SDK bereitgestellt wird. Die Ausgabe der Richtlinie an STDOUT darf nur die Validierungsantwort enthalten.
====
...
{admc-short-name} Richtlinien verwenden zwei Funktionen, die Einstellungen verwalten:
