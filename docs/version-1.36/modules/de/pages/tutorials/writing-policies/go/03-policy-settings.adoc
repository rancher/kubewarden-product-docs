...
Eine {admc-short-name}-Richtlinie bietet zwei verschiedene Funktionen, die die Richtlinieneinstellungen als Eingabe erhalten:
...
* `validate_settings`: Rufen Sie diese Funktion auf, wenn die Richtlinie erstmals von {admc-short-name} geladen wird.
...
Alle {admc-short-name}-Richtlinien müssen die Einstellungsvalidierung implementieren.
...
Sie können sehen, dass die Funktion die Hilfsfunktionen nutzt, die von https://github.com/kubewarden/policy-sdk-go[{admc-short-name}'s SDK] bereitgestellt werden.
