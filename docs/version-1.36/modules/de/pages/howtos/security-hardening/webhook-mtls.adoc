...
Webhooks des {admc-name}-Stacks aktivieren, wenn Sie https://k3s.io/[K3s] als Ihre
...
Webhooks des {admc-short-name} und dem Kubernetes API-Server abzusichern.
...
Das Zertifikat der Root-CA, die das Kubernetes-Client-Zertifikat ausgestellt hat, muss dem {admc-short-name} Stack zur Verfügung stehen.
...
Installieren Sie den {admc-short-name} Stack wie im xref:/quick-start.adoc[Schnellstartleitfaden] beschrieben. Befolgen Sie alle Schritte, aber stellen Sie beim Installieren des `kubewarden-controller` Helm-Charts sicher, dass Sie die folgenden Werte aktivieren:
...
Sobald dieser Befehl abgeschlossen ist, sichert mTLS die Installation des {admc-short-name} Stacks und seiner Webhooks.