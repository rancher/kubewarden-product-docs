...
:description: Beheben von Problemen mit der Zertifikatsrotation des Policy Servers in {admc-name}, beginnend mit Version 1.16.
...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "Zertifikate des Policy Servers"]
:sidebar_label: Zertifikatsrotation
:sidebar_position: 10
:current-version: {page-origin-branch}

[IMPORTANT]
.Umgehungslösung für {admc-name} v1.16.0 und früher
====

Diese Umgehungslösung ist nur für {admc-short-name} v1.16.0 und früher erforderlich.
Ab v1.17.0 erneuert der Controller die Zertifikate des Policy Servers automatisch.

====

Während des Freigabeprozesses für v1.14 entdeckte der {admc-short-name} einen Fehler im Zusammenhang mit der Zertifikatsrotation des Policy Servers. Die Root-CA-Konfiguration sieht eine Laufzeit von 10 Jahren vor, aber jedes Zertifikats-Secret des Policy Servers hat eine Laufzeit von einem Jahr. Der Controller ist jedoch derzeit nicht in der Lage, sie automatisch zu erneuern.

In der Version v1.14 stellte der {admc-short-name} sicher, dass die Secrets des Policy Servers mit einer Laufzeit von 10 Jahren erstellt werden.
