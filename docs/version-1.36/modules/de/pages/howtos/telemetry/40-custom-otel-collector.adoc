...
:description: Konfigurieren Sie den {admc-name}, um Telemetriedaten sicher an einen benutzerdefinierten OpenTelemetry Collector zu senden, der innerhalb des Clusters bereitgestellt wird.
...
Dieser Leitfaden erklärt, wie Sie {admc-short-name} konfigurieren, um Telemetriedaten an einen bereits im Cluster bereitgestellten OpenTelemetry Collector zu senden.
...
Sie benötigen eine verschlüsselte Kommunikation zwischen den {admc-name}-Komponenten und dem Collector. Sie können https://cert-manager.io/[cert-manager] verwenden, um alle für die sichere Kommunikation erforderlichen Zertifikate zu verwalten.

OpenTelemetry Collector-Traces werden an eine https://www.jaegertracing.io/[Jaeger]-Instanz gesendet.

Der {admc-short-name}-Stack sendet Metriken an den OpenTelemetry Collector.
...
Sie richten die Kommunikation zwischen den {admc-short-name}-Komponenten und dem OpenTelemetry Collector mit mTLS ein.
...
Der Prometheus-Dienstmonitor erhält die {admc-short-name} Metriken, indem er den OpenTelemetry-Collector im `kubewarden` Namespace abruft.
...
Sie sichern die Kommunikation zwischen dem {admc-short-name} Stack und dem OpenTelemetry Collector mit mTLS. Die Kommunikation zwischen dem OpenTelemetry Collector und Jaeger ist jedoch nicht gesichert, um die Komplexität des Beispiels zu reduzieren.

== Installieren Sie den {admc-short-name} Stack

Wenn der OpenTelemetry Collector läuft, können Sie {admc-short-name} auf die übliche Weise bereitstellen.

Sie müssen die {admc-short-name} Komponenten konfigurieren, damit sie Ereignisse und Metriken an den OpenTelemetry Collector senden.
...
Sie können die von {admc-short-name} generierten Trace-Ereignisse über die Jaeger-Web-UI sehen. Sie sind unter dem `kubewarden-policy-server` Dienst gruppiert:
