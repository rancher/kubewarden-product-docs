...
:description: Verwenden Sie Hauler, um {admc-name} in Air Gap-Umgebungen zu installieren.
...
Dieser Leitfaden zeigt Ihnen, wie Sie {admc-name} in Air Gap-Umgebungen mit link:https://docs.hauler.dev/docs/intro[Hauler] installieren. Hauler ist ein Tool, das Benutzern hilft, Workloads in Air Gap-Umgebungen auszuführen. Es bewegt die erforderlichen Ressourcen für Apps in diese Air Gap-Umgebungen.

{admc-short-name} stellt eine Manifestdatei mit allen erforderlichen Ressourcen zur Verfügung, um {admc-short-name} in einer privaten Umgebung auszuführen. Diese Dokumentation beschreibt, wie Sie es verwenden können. Konsultieren Sie die Hauler-Dokumentation, um mehr darüber zu erfahren.

Der grundlegende Arbeitsablauf mit dem Hauler-Manifest ist:

. Holen Sie es von einer {admc-short-name}-Release-Seite
. Laden Sie alle Ressourcen, die erforderlich sind, um {admc-short-name} auszuführen, in einen lokalen Speicher
...
Laden Sie `hauler_manifest.yml` von der {admc-short-name} link:https://github.com/kubewarden/helm-charts/releases/[Release-Seite] herunter.
...
== Generieren Sie die TAR-Datei mit allen {admc-short-name} Ressourcen.
...
{admc-short-name} Containerbilder unterstützen die Architekturen x86_64 und ARM. Daher können Sie, wenn Sie die Ressource in die Datei speichern, Warnmeldungen wie diese sehen:
...
Jetzt, da Sie alle {admc-short-name} Ressourcen in `kubewarden-resources.tar.zst` haben, kopieren Sie es in Ihre Air Gap-Umgebung und laden Sie es dort in den Hauler-Speicher:
...
Jetzt sind alle Ressourcen, die zur Installation von {admc-short-name} erforderlich sind, im Hauler-Speicher in Ihrer isolierten Umgebung.
...
Dies startet ein privates Registry unter der Adresse `localhost:5000`. Von diesem Punkt an können Sie andere Befehle wie Skopeo verwenden, um alle Containerbilder, Richtlinienmodule und Helm-Charts, die von {admc-short-name} verwendet werden, in Ihr privates Registry zu kopieren.

== Installieren Sie {admc-short-name}

Jetzt, da Ihr privates Registry alles Erforderliche hat, können Sie {admc-short-name} installieren. Der Unterschied zu einer Standardinstallation von {admc-short-name} besteht darin, dass Sie das Registry in den Containerbildern und Richtlinien auf das private Registry ändern müssen. Zusätzlich müssen Sie die Helm-Charts aus OCI-Artefakten installieren.
...
Jetzt erstellen Sie {admc-short-name} Richtlinien in Ihrem Cluster. Richtlinien müssen in Ihrem privaten Registry verfügbar sein.
