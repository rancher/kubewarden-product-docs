...
Die https://github.com/kubernetes/community/tree/master/sig-security[Sicherheits-Sondergruppe (SIG) von Kubernetes] hat ein Bedrohungsmodell für die Zugangssteuerung für Kubernetes definiert. Das {admc-name} Team bewertet kontinuierlich {admc-short-name} anhand dieses Bedrohungsmodells und arbeitet daran, sichere Voreinstellungen bereitzustellen. Es wird empfohlen, dass {admc-short-name} Administratoren das Bedrohungsmodell lesen und verstehen und es verwenden, um bei Bedarf ihr eigenes, situationsspezifisches Bedrohungsmodell zu entwickeln.
...
Dies ist das Standardverhalten von {admc-short-name}.

Das Schließen im Fehlerfall bedeutet, dass der API-Server die Anfrage standardmäßig ablehnt, wenn {admc-short-name} aus irgendeinem Grund nicht mehr reagiert oder abstürzt.
Dies gilt selbst dann, wenn die Anfrage normalerweise von {admc-short-name} akzeptiert wird.
...
Der Großteil von RBAC liegt nicht im Rahmen der aktuellen Diskussion. Die folgenden Informationen werden in Kürze bereitgestellt, um {admc-short-name} Benutzern zu helfen:
...
Da der Webhook TLS-Verschlüsselung für den gesamten Datenverkehr verwendet, ist {admc-short-name} sicher.
...
Der Großteil des RBAC liegt außerhalb des Anwendungsbereichs dieser Entscheidung. Das {admc-short-name} Team hat jedoch das Ziel:
...
* Konfigurieren Sie den Cluster mit mTLS-Authentifizierung für die Webhooks und aktivieren Sie die mTLS-Funktion im {admc-short-name} Stack. Alternativ können Sie mTLS mit einem CNI-Setup verwenden, das Netzwerk-Richtlinien unterstützt.

* Standardmäßig haben {admc-short-name} Richtlinien keinen Netzwerkzugang und laufen in einer restriktiven Umgebung, die den externen Zugriff auf Webhooks streng kontrolliert.

== {admc-short-name} Bedrohungen

=== {admc-short-name} Bedrohung 1 - Vertrauensaufbau für den Zulassungscontroller

==== Szenario

Vorausgesetzt, es handelt sich um einen vertrauenswürdigen, aber neuen Kubernetes-Cluster, kann ein Angreifer den {admc-short-name}-Stack kompromittieren, bevor die Implementierung und Durchsetzung aller Sicherheitsrichtlinien erfolgt.
 ...
 ** Richtlinien-Server
 ** irgendeine der {admc-short-name} Abhängigkeiten
 ** alle optionalen Abhängigkeiten (Grafana, Prometheus und andere)
* durch Kompromittierung des Helm-Chart-Payloads

==== Risikominderung

. {admc-short-name} bietet ein Software Bill Of Materials, das alle benötigten Bilder auflistet. Dies unterstützt Zero-Trust. Der Kubernetes-Administrator muss die {admc-short-name} Bilder, die Bilder der Abhängigkeiten und die Charts aus dem Kubernetes-Cluster in einer vertrauenswürdigen Umgebung überprüfen. Sie können dies zum Beispiel mit `cosign` tun. Übrigens ist dies Teil der Implementierung, die für Air-Gapped-Installationen erforderlich ist.
. Verwenden Sie signierte Helm-Charts und verifizierte Digests anstelle von Tags für {admc-short-name}
