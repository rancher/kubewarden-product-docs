...
:description: Erforschen Sie die Kubernetes-Funktionen mithilfe der kontextsensitiven Richtlinien von {admc-name} und des waPC-Protokolls, um auf Clusterressourcen zuzugreifen.
...
Die kontextsensitiven Richtlinien von {admc-name} benötigen Zugriff auf Ressourcen aus dem Kubernetes-Cluster, in dem sie ausgeführt werden.
Dafür stellen die {admc-short-name} SDKs Funktionen zur Verfügung, die das waPC-Kommunikationsprotokoll verwenden, um mit dem Hostsystem zu kommunizieren und Daten über den Cluster anzufordern.
