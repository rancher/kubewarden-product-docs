...
:description: Publique {admc-name} políticas no Artifact Hub criando um layout de repositório Git bem formatado e configurando o GitHub Actions para empacotamento automatizado, facilitando a integração contínua.
...
As políticas {admc-name} podem ser publicadas no Artifact Hub e tornadas descobríveis para o amplo público de usuários do CNCF.
...
Este documento foca nos passos necessários para tornar uma política {admc-short-name} descobrível no Artifact Hub.
...
* Escolha _"{admc-short-name} políticas"_ como tipo.
