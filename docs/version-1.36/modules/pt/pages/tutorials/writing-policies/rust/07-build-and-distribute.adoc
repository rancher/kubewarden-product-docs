...
:description: Siga estas etapas essenciais para construir e distribuir políticas de forma eficaz para sua configuração {admc-name}.
...
{admc-name} documenta a distribuição de políticas na seção
xref:../../../explanations/distributing-policies.adoc[distribuindo políticas].

== Mais exemplos

Você pode encontrar mais políticas Rust {admc-short-name} no espaço do GitHub de {admc-short-name}.
...
Os repositórios de políticas {admc-short-name}, mostrados por essa consulta, têm Ações do GitHub que automatizam as seguintes tarefas:
