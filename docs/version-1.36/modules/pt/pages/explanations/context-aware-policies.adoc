...
:keywords: ["{admc-name}", "Kubewarden", "política sensível ao contexto", "clusteradmissionpolicy", "admissionpolicy", "admissionrequest", "administrador do cluster"]
...
[NOTE]
====
Políticas sensíveis ao contexto estão disponíveis apenas em versões {admc-name} ≥ v1.6.0.
====
...
Por padrão, {admc-short-name} bloqueia todos os recursos do cluster. Um administrador {admc-name} define quais recursos do Kubernetes cada política sensível ao contexto pode ler. `contextAwareResources` para fazer isso.
...
Os autores de políticas fornecem listas de recursos do Kubernetes para sua política sensível ao contexto. Os autores de políticas fazem isso anotando a política. Os administradores {admc-name} visualizam os metadados da política usando o comando `kwctl inspect`. Eles podem obter uma lista de recursos aos quais a política precisa de acesso. Um administrador usa esta lista para preencher a definição `ClusterAdmissionPolicy`.

[WARNING]
====

Para prevenir abusos do sistema, os administradores {admc-name} devem revisar os recursos que a política acessa.
