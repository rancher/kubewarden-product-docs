...
= {admc-name} em um Ambiente de Grande Escala
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Configure {admc-name}s com {admc-name} para garantir alta disponibilidade e desempenho em ambientes de grande escala exigentes.
:keywords: [kubewarden, "{admc-name}", kubernetes, policyservers, produção]
...
Esta seção detalha uma implantação do mundo real de {admc-name} em um ambiente de grande escala exigente. Ela ilustra como configurar {admc-short-name} para alta disponibilidade e desempenho e o que esperar sob carga pesada.

[IMPORTANT]
====
Se você quiser ver mais dicas sobre como executar {admc-short-name} em produção, confira a documentação de xref:/howtos/production-deployments.adoc[Implantações em produção].
====
...
== {admc-short-name} Configuração

Para atender às demandas deste ambiente, {admc-short-name} é configurado com foco no isolamento de carga de trabalho e alta disponibilidade.
