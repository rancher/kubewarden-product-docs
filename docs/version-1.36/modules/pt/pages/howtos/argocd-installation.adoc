...
Se você está usando o ArgoCD para gerenciar seus recursos do Kubernetes, pode querer instalar {admc-name} usando o ArgoCD. Isso é possível, mas há certas considerações. Desde a versão {admc-short-name} v1.17.0, a dependência do cert-manager foi removida. Há um novo reconciliador de certificados que renova automaticamente os certificados. Isso significa que o controlador {admc-short-name} cuida da criação e renovação dos certificados para você. Isso inclui tanto a CA raiz quanto todos os certificados de webhook.
...
No exemplo, o aspecto chave é a configuração `ignoreDifferences` para o aplicativo do controlador {admc-short-name}. O restante do conteúdo fornece contexto sobre como configurar o aplicativo ArgoCD e fornece um exemplo completo.
