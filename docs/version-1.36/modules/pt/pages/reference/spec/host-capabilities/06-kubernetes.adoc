...
:description: Explore os recursos do Kubernetes usando as políticas contextuais do {admc-name} e o protocolo waPC para acessar os recursos do cluster.
...
As políticas contextuais {admc-name} requerem acesso aos recursos do cluster Kubernetes onde estão sendo executadas.
Para isso, os SDKs {admc-short-name} expõem funções que utilizam o protocolo de comunicação waPC para se comunicar com o sistema host solicitando dados sobre o cluster.
